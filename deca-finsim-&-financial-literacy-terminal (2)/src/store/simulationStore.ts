...
export type PlatformWing = 'DECA' | 'FINANCE' | 'SPLIT';
...
interface SimulationStore {
  // Navigation & Platform Wings State
  activeWing: PlatformWing;
  setActiveWing: (wing: PlatformWing) => void;
  activeTab: NavTab;
  ...
  // DECA Actions
  selectRoleplayTopicById: (id: string) => void;
...
const INITIAL_CASE_STUDY: DecaCaseStudy = DECA_ROLEPLAY_SCENARIOS[0];
...
const INITIAL_RUBRIC: DecaRubricCriterion[] = [
  { id: 'rub-1', indicator: 'CR:009 - Adversarial Customer De-escalation', maxPoints: 25, earnedPoints: 25, feedback: 'Calmly isolated the public confrontation into a private manager lounge, eliminating lobby disruption.' },
  { id: 'rub-2', indicator: 'EI:015 - Emotional Intelligence & Active Listening', maxPoints: 25, earnedPoints: 24, feedback: 'Validated the VIP guest urgency without defensiveness; protected front-line staff from unearned discipline.' },
  { id: 'rub-3', indicator: 'HT:008 - Hospitality Service Recovery Execution', maxPoints: 25, earnedPoints: 24, feedback: 'Awarded $150 rooftop dining voucher and 20,000 points while expediting priority luggage delivery.' },
  { id: 'rub-4', indicator: 'SM:001 - Executive Communication & Brand Defense', maxPoints: 25, earnedPoints: 23, feedback: 'Delivered an impressive 3-step action timeline that turned a hostile confrontation into a brand-building moment.' },
];
...
export const useSimulationStore = create<SimulationStore>((set, get) => ({
  ...
  setActiveWing: (wing) => {
    ...
    let nextTab: NavTab = currentTab;
    if (wing === 'DECA') {
      const decaTabs: NavTab[] = ['deca_hub', 'deca_exams', 'deca_roleplay', 'deca_reports', 'deca_rubrics'];
      if (!decaTabs.includes(currentTab)) {
        nextTab = 'deca_hub';
      }
    } else if (wing === 'FINANCE') {
      const finTabs: NavTab[] = ['finance_hub', 'stock_trading', 'real_estate', 'corporate_finance', 'portfolio', 'sql_schema'];
      if (!finTabs.includes(currentTab)) {
        nextTab = 'finance_hub';
      }
    } else {
      nextTab = 'split_hub';
    }
    set({ activeWing: wing, activeTab: nextTab });
  },
  setActiveTab: (tab) => {
    let wing = get().activeWing;
    const decaTabs: NavTab[] = ['deca_hub', 'deca_exams', 'deca_roleplay', 'deca_reports', 'deca_rubrics'];
    const finTabs: NavTab[] = ['finance_hub', 'stock_trading', 'real_estate', 'corporate_finance', 'portfolio', 'sql_schema'];

    if (tab === 'split_hub' || tab === 'bento') {
      wing = 'SPLIT';
    } else if (decaTabs.includes(tab)) {
      wing = 'DECA';
    } else if (finTabs.includes(tab)) {
      wing = 'FINANCE';
    }
    set({ activeTab: tab, activeWing: wing });
  },
  ...
  usedCaseStudyIds: ['cs-hosp-01'],
  roleplayMatrix: {
    clientProblemNotes: 'A VIP corporate executive (Mr. Harrison) is shouting, filming staff, and causing a public scene in the lobby over a 25-minute luggage delivery delay.',
    systemConstraintsNotes: 'Discretionary comp limit is $250 without GM approval. Presidential Suites are 100% full. Front desk staff cannot be disciplined without verified cause.',
    piNotes: {
      'CR:009': 'De-escalation: Speak with a calm, low vocal tone. Move Mr. Harrison into the private Executive VIP Lounge away from arriving guests.',
      'EI:015': 'Emotional Intelligence: Validate his urgency for his business conference without getting defensive or admitting legal fault.',
      'HT:008': 'Service Recovery: Track luggage with courier dispatcher, issue a $150 rooftop dining voucher, and credit 20,000 bonus loyalty points to his profile.',
      'SM:001': '3-Step Plan: 1) Immediate Private Lounge Comfort & Drink, 2) Priority Bellman Luggage Delivery, 3) Personal GM Handwritten Welcome Note.',
    },
    pitchScript: `Good evening Mr. Harrison. My name is Alex, Assistant Front Office Manager. First, let me personally apologize for the luggage delay and the stress this has caused before your conference. 

I completely understand the critical urgency of having your presentation materials on time. Let us step right into our quiet Executive VIP Lounge where I can offer you a beverage and personally resolve this.

Here is my immediate 3-step action plan for you tonight:
1. Luggage Priority Rush: Our bell captain has located your bags on our express transfer shuttle; they are en route right now and will be hand-delivered directly to your room in 15 minutes.
2. Executive Hospitality Restitution: While our team unpacks your luggage, please enjoy a complimentary dinner at our rooftop steakhouse ($150 credit) on us tonight.
3. VIP Loyalty Credit: As our token of gratitude for your patience, I have already deposited 20,000 bonus reward points directly into your loyalty profile.

I will personally oversee your stay this weekend to ensure flawless service. Thank you for your continued loyalty to Grand Azure.`,
  },
  roleplayTimerSeconds: 900, // 15m 00s
  isTimerRunning: false,
  rubricCriteria: INITIAL_RUBRIC,
  studentReportText: `# HIGH SCHOOL DECA STRATEGIC BUSINESS & OPERATIONS PLAN
## EXECUTIVE SUMMARY
This comprehensive Strategic Business Plan outlines a high-school student-run retail enterprise ("Summit Spirit & Apparel Store"). By leveraging modern financial management tools, double-entry accounting fundamentals (Assets = Liabilities + Equity), break-even pricing models, and disciplined cash flow forecasting, our enterprise delivers high-quality school spirit gear to students and faculty while generating consistent net profits to fund chapter student scholarships.

## 1. MARKET & ECONOMIC ANALYSIS
- **Target Market**: 1,400 High School Students, 95 Faculty Members, and Athletic Booster Club Supporters.
- **Economic Principles Applied**:
  - *Law of Demand*: Optimized spirit hoodie pricing at $30 (maximizing unit volume and gross revenue).
  - *Break-Even Analysis*: Fixed setup costs of $600 with unit variable cost of $12 requires selling 34 units to reach break-even.
  - *Inflation Hedge*: Locked in bulk wholesale fabric contracts for 12 months to protect margins against raw material price increases.

## 2. FINANCIAL STATEMENTS & CASH FLOW PROJECTIONS
- **Year 1 Projected Revenue**: $48,000
- **Cost of Goods Sold (COGS)**: $24,000 (50% Gross Margin)
- **Gross Profit**: $24,000
- **Operating Expenses**: $9,600 (Store rent contribution, marketing flyers, POS software)
- **Net Operating Income**: $14,400 (30% Net Profit Margin)

## 3. INTERNAL CONTROLS & ETHICAL SAFEGUARDS
- Daily register reconciliation with dual-signature cash counting sheets.
- Implementation of modern Square/Stripe POS terminals with multi-factor authentication (MFA) to prevent credit fraud.
- Quarterly physical inventory audits to reduce inventory shrinkage below 1.0%.`,
  ...
  selectRoleplayTopicById: (id: string) => {
    const scenario = DECA_ROLEPLAY_SCENARIOS.find((s) => s.id === id);
    if (!scenario) return;
    const { usedCaseStudyIds } = get();
    const updatedUsedIds = usedCaseStudyIds.includes(scenario.id)
      ? usedCaseStudyIds
      : [...usedCaseStudyIds, scenario.id];

    const freshPiNotes: Record<string, string> = {};
    scenario.performanceIndicators.forEach((pi) => {
    ...
    set({
      ...
      roleplayTimerSeconds: (scenario.timeLimitMinutes || 15) * 60,
      isTimerRunning: false,
    });
  },
